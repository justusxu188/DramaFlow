...
**一句话总结给你的用户：** 跑通 real 模式 = 用 deploy 目录里这份 `deploy-real-mode.sh` 做服务器初始化 + 把第 2 节的 10 条火山控制台凭据/Endpoints 完整填到 `/etc/frameflow.env`，然后 `systemctl restart frameflow.service`。脚本和 README 会兜底做备份、自检和健康检查。
